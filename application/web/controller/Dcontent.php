 ...
 */
class Dcontent extends BasicAdmin
{
...
//        print_r($this->_teacher());exit;
        $dmcid = $this->request->get('dmcid');
//        $chapter_id = db($this->chapter)->where("degree_id","=",$dmcid)->column("id");
        $db = Db::name($this->table)->where(['is_deleted' => '0'])->where('chapter_id','=',$dmcid);
...
//        }
    ...
    {
        $dmcid = $this->request->get('dmcid');
        return db($this->chapter)->where(['is_deleted' => '0','status'=>'1','degree_id'=>$dmcid])->select();
    }
...
//            $chapter = $this->_chapter();
            $row['chapter_name'] = db($this->chapter)->where('id', '=', $dmcid)->value('name');
            $row['chapter_id'] = db($this->chapter)->where('id', '=', $dmcid)->value('id');
